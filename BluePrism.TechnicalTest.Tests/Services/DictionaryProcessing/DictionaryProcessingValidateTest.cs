...
namespace BluePrism.TechnicalTest.Tests.Services.DictionaryProcessing
{
    public class DictionaryProcessingValidateTest
    {
        ...
        private ILogger<DictionaryProcessingService> _logger;

        [SetUp]
        public void Setup()
        {
            _logger = Mock.Of<ILogger<DictionaryProcessingService>>();

            _dictionaryService = new DictionaryProcessingService(new ProcessFileInputValidator(), _logger);
